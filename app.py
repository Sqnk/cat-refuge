...
# En local, DATA_DIR = BASE_DIR. Sur Render, on ajoutera une variable d'env DATA_DIR=/var/data
...
class VaccineRecord(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    cat_id = db.Column(db.Integer, db.ForeignKey('cat.id'), nullable=False)
    vaccine_name = db.Column(db.String(200), nullable=False)  # texte libre (compatible historique)
...
def compute_vaccine_alerts(days=30):
    ...
    ⚠️ N'inclut QUE les types pour lesquels le chat a AU MOINS un enregistrement.
    (Les 'jamais vaccinés' pour un type sont ignorés ici, comme demandé)
    """
    today = date.today()
...
def cat_missing_vaccine_q(query, vaccine_name):
    """
    Filtre 'vaccin manquant' = pas de dose dans les 12 derniers mois.
    (Ce filtre côté liste peut inclure 'jamais vaccinés', c'est voulu pour la recherche.)
    """
...
# ====================== Rendez-vous (liste / new / edit / delete) ======================
...
# ====================== API Calendrier (GET + POST select + PATCH drag) ======================
...
def api_appointments():
    if request.method == 'GET':
        ...
        events = []
        for a in appts:
            title = a.location or "Rendez-vous"   # affichage = lieu
            events.append({
                "id": a.id,
                "title": title,
...
# ====================== Debug & Seed ======================
@app.route('/api/appointments/debug')
def api_appointments_debug():
    appts = Appointment.query.order_by(Appointment.date).all()
    return jsonify({
        "count": len(appts),
        "items": [
            {
                ...
                "date_iso": a.date.strftime('%Y-%m-%dT%H:%M:%S'),
                "cats": [c.name for c in a.cats],
                "employees": [e.name for e in a.employees]
            } for a in appts
        ]
...
def dev_seed():
    ...
    a = Appointment(location="Test Cabinet", date=dt, notes="seed")
    db.session.add(a)
    db.session.commit()
    return jsonify({"ok": True, "created_id": a.id, "start": dt.strftime('%Y-%m-%dT%H:%M:%S')})

# ====================== Recherche plein texte notes ======================
...
if __name__ == '__main__':
    with app.app_context():
        db.create_all()
        # Seed par défaut des types de vaccins si table vide
